...
int main() {
    ...
    {
        std::ofstream(root / "config/path_policy.json")
            << R"({"protectedRoots":["/Atlas"],"generatedRoots":["/Generated"],"archiveRoots":["/Archive"],"sandboxRoots":["/Local"],"externalSyncRoots":["/ExternalSync"]})";
        std::ofstream(root / "config/session_capabilities.json")
            << R"({"schemaVersion":1,"modes":{"observer":["read_repo"],"editor":["read_repo","write_generated","archive_intake","run_build"],"admin_local":["*"]}})";
        std::ofstream(root / "config/tool_allowlist.json")
            << R"({"schemaVersion":1,"tools":[{"toolId":"cmake_build","exe":"cmake.exe","allowedArgs":["--build","--preset","--config"],"workingRootPolicy":"repo_or_build_dir_only","timeoutSeconds":300,"networkAccess":false}]})";
        std::ofstream(root / "config/archive_intake_policy.json")
            << R"({"schemaVersion":1,"autoExtractZips":true,"quarantine":true,"generateReports":true,"allowedRepoRootNames":["Atlas","Archive"]})";
    }
